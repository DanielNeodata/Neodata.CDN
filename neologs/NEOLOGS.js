var _NEOLOGS = {
	...
	onModalAlert: function (_title, _body, _class) {
		...
		_NEOLOGS.onDestroyModal("#alterModal");
	...
	onErrHandler: function (msg) {
		...
		_NEOLOGS.onModalAlert("System alert", _text, "warning");
	},
	...
	onAuthenticate: function () {
		return new Promise(
			function (resolve, reject) {
				try {
					if (_NEOLOGS._username == "" || _NEOLOGS._password == "") {
						_NEOLOGS.onErrHandler("Debe proveer un 'username' y 'password'");
						return false;
					}
					var _auth = { "username": _NEOLOGS._username, "password": _NEOLOGS._password, "id": _NEOLOGS._id_application };
					_NEOAUTHENTICATION.UiAuthenticate(_auth)
						.then(function (data) {
							if (data.status == "OK") {
								_NEOLOGS._id_application = data.records[0].id_application;
								_NEOLOGS._application = data.records[0].application;
								_NEOLOGS._id_user = data.records[0].id_user;
								_NEOLOGS._username = data.records[0].username;
								_NEOLOGS._token = data.token;
								_NEOLOGS._token_type = "";
								resolve(data);
							} else {
								reject(data);
							}
						...
						.catch(function (rex) {
							_NEOLOGS.onErrHandler(rex);
							reject(rex);
						});
				} catch (err) {
					_NEOLOGS.onErrHandler(err.message);
					reject(err);
				}
			});
	...
	onState: function () {
		return new Promise(
			function (resolve, reject) {
				_NEOLOGS.onAuthenticate().then(function (data) {
					_NEOLOGS.State({}).then(function (data) {
						if (data.status == "OK") {
							resolve(data);
						} else {
							reject(data);
	...
	onMonitoring: function (_params) {
		return new Promise(
			function (resolve, reject) {
				_NEOLOGS.onAuthenticate().then(function (data) {
					_NEOLOGS.Monitoring(_params).then(function (data) {
						if (data.status == "OK") {
							resolve(data);
						} else {
							reject(data);
	...
	State: function (_json) {
		return new Promise(
			function (resolve, reject) {
				_json.method = "State";
				_NEOLOGS._waiter = true;
				_NEOLOGS.ExecuteDirect(_json, null).then(function (data) { resolve(data); }).catch(function (err) { reject(err); });
			});
	},
	...
	formatFixedParameters: function (_json) {
		_json["id_user"] = _NEOLOGS._id_user;
		_json["id_application"] = _NEOLOGS._id_application;
		_json["token"] = _NEOLOGS._token;
		if (_json["server"] == undefined) { _json["server"] = _NEOLOGS._SERVER; }
		...
		return new Promise(
			function (resolve, reject) {
				try {
					...
					_NEOLOGS.Execute(_json).then(function (datajson) {
	...
	Execute: function (_json) {
		return new Promise(
			function (resolve, reject) {
				try {
					var _params = _NEOLOGS.formatFixedParameters(_json);
