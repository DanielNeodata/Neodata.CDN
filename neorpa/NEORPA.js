var _NEORPA = {
	...
	onModalAlert: function (_title, _body, _class) {
		...
		_NEORPA.onDestroyModal("#alterModal");
	...
	onErrHandler: function (msg) {
		...
		_NEORPA.onModalAlert("System alert", _text, "warning");
	},
	...
	onAuthenticate: function () {
		return new Promise(
			function (resolve, reject) {
				try {
					if (_NEORPA._username == "" || _NEORPA._password == "") {
						_NEORPA.onErrHandler("Debe proveer un 'username' y 'password'");
						return false;
					}
					var _auth = { "username": _NEORPA._username, "password": _NEORPA._password, "id": _NEORPA._id_application };
					_NEOAUTHENTICATION.UiAuthenticate(_auth)
						.then(function (data) {
							if (data.status == "OK") {
								_NEORPA._id_application = data.records[0].id_application;
								_NEORPA._application = data.records[0].application;
								_NEORPA._id_user = data.records[0].id_user;
								_NEORPA._username = data.records[0].username;
								_NEORPA._token = data.token;
								_NEORPA._token_type = "";
								resolve(data);
							} else {
								reject(data);
						...
						.catch(function (rex) {
							_NEORPA.onErrHandler(rex);
							reject(rex);
						});
				} catch (err) {
					_NEORPA.onErrHandler(err.message);
					reject(err);
				}
			});
	...
	onState: function () {
		return new Promise(
			function (resolve, reject) {
				_NEORPA.onAuthenticate().then(function (data) {
					_NEORPA.State({}).then(function (data) {
						if (data.status == "OK") {
							resolve(data);
						} else {
							reject(data);
	...
	onMonitoring: function (_params) {
		return new Promise(
			function (resolve, reject) {
				_NEORPA.onAuthenticate().then(function (data) {
					_NEORPA.Monitoring(_params).then(function (data) {
						if (data.status == "OK") {
							resolve(data);
						} else {
							reject(data);
	...
	State: function (_json) {
		return new Promise(
			function (resolve, reject) {
				_json.method = "State";
				_NEORPA._waiter = true;
				_NEORPA.ExecuteDirect(_json, null).then(function (data) { resolve(data); }).catch(function (err) { reject(err); });
			});
	},
	...
	formatFixedParameters: function (_json) {
		_json["id_user"] = _NEORPA._id_user;
		_json["id_application"] = _NEORPA._id_application;
		_json["token"] = _NEORPA._token;
		if (_json["server"] == undefined) { _json["server"] = _NEORPA._SERVER; }
		...
		return new Promise(
			function (resolve, reject) {
				try {
					...
					_NEORPA.Execute(_json).then(function (datajson) {
	...
	Execute: function (_json) {
		return new Promise(
			function (resolve, reject) {
				try {
					var _params = _NEORPA.formatFixedParameters(_json);
